...
class UserInterface:
    ...
    def __init__(
        ...
        self.instrument = exaspim.Exaspim(
            config_filepath=config_filepath, simulated=simulated
        )
        self.simulated = simulated
        self.cfg = self.instrument.cfg
        self.viewer = napari.Viewer(
            title="exaSPIM control", ndisplay=2, axis_labels=("x", "y")
        )
        ...
        main_widgets = {
            "main_block": self.instrument_params.create_layout(
                struct="V",
        ...
        laser_window.setWidget(
            self.laser_parameters.create_layout(struct="H", **laser_widget)
        )
        ...
        liveview_widget = self.livestream_parameters.create_layout(
            struct="V",
        ...
        self.viewer.window.add_dock_widget(
            self.livestream_parameters.create_layout(
                struct="V", live=liveview_widget, tab=tabbed_widgets
            ),
            name=" ",
        )  # Adding tabs to window

        self.viewer.window.add_dock_widget(
            instr_params_window, name="Instrument Parameters", area="left"
        )
        self.viewer.window.add_dock_widget(
            laser_window, name="Laser Current", area="bottom"
        )
    ...
    def instrument_params_widget(self):
        self.instrument_params = InstrumentParameters(
        ...
        tabbed_widgets.addTab(
            self.instrument_params.brain_orientation_widget(), "Brain Orientation"
        )
        x_game_mode = True
        widgets = {
            "config_properties": self.instrument_params.scan_config(
                self.cfg, x_game_mode
            ),
        }
    ...
    def livestream_widget(self):
        ...
        self.livestream_parameters = Livestream(
            self.viewer, self.cfg, self.instrument, self.simulated
        )
    ...
    def volumeteric_acquisition_widget(self):
        ...
        self.vol_acq_params = VolumetericAcquisition(
            self.viewer, self.cfg, self.instrument, self.simulated
        )
        widgets = {
