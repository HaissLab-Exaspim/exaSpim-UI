...
class Livestream(WidgetBase):
    ...
    def __init__(self, viewer, cfg, instrument, simulated: bool):
        ...
        self.set_volume = (
            {}
        )  # Holds widgets related to setting volume limits during scan
        self.move_stage = {}
        ...
        self.scale = [
            self.cfg.cfg["tile_specs"]["x_field_of_view_um"]
            / self.cfg.sensor_column_count,
            self.cfg.cfg["tile_specs"]["y_field_of_view_um"]
    ...
    def update_positon(self, index):
        ...
        if index == 0:
            ...
            try:  # TODO: This is hack for when tigerbox reply is split e.g. '3\r:A4 -76 0 \n'
                ...
                self.stage_position["n"] = self.instrument.tigerbox.get_position("n")[
                    "N"
                ]
                # Update stage labels if stage has moved
                for direction in directions:
                    self.pos_widget[direction].setValue(
                        int(self.stage_position[direction] * 1 / 10)
                    )
            except Exception:
                pass
    ...
    def liveview_widget(self):
        ...
        self.live_view["checkboxes"] = self.create_layout(
            struct="H", **self.live_view_checks
        )
    ...
    def show_crosshairs(self, state):
        ...
        if state == 2:
            ...
            vert_line = np.array(
                [
                    [0, self.cfg.tile_size_x_um * 0.5],
                    [self.cfg.tile_size_y_um, self.cfg.tile_size_x_um * 0.5],
                ]
            )
            horz_line = np.array(
                [
                    [self.cfg.tile_size_y_um * 0.5, 0],
                    [self.cfg.tile_size_y_um * 0.5, self.cfg.tile_size_x_um],
                ]
            )
            l = [vert_line, horz_line]  # noqa: E741
            color = ["blue", "green"]

            shapes_layer = self.viewer.add_shapes(
                l, shape_type="line", edge_width=30, edge_color=color, name="Crosshair"
            )
    ...
    def start_live_view(self):
        ...
        wavelength = [
            int(item.text()) for item in self.live_view["wavelength"].selectedItems()
        ]
    ...
    def dissect_image(self, args):
        ...
        try:
            ...
            container[:chunk, chunk : chunk + len] = image[0][
                :chunk, lower_col:upper_col
            ]  # Top
            container[-chunk:, chunk : chunk + len] = image[0][
                -chunk:, lower_col:upper_col
            ]  # bottom
    ...
    def sample_stage_position(self):
        ...
        for direction in directions:
            self.pos_widget[direction + "label"], self.pos_widget[direction] = (
                self.create_widget(
                    self.stage_position[direction] * 1 / 10,
                    QSpinBox,
                    f"{direction} [um]:",
                )
            )
        ...
        self.pos_widget["volume_widgets"] = self.create_layout(
            struct="V", **self.set_volume
        )
    ...
    def color_change_list(self, item):
        ...
        if item.isSelected():
            ...
            item.setBackground(
                QtGui.QColor(65, 72, 81, 255)
            )  # Napari widget default color
            ...
            if not self.instrument.stage_lock.locked():
                with self.instrument.stage_lock:
                    try:
                        ...
                        self.sample_pos["n"] = self.instrument.tigerbox.get_position(
                            "n"
                        )["N"]
                        sleep(0.01)
                        ...
                        if moved:
                            self.update_slider(
                                self.sample_pos
                            )  # Update slide with newest z depth
    ...
    def move_stage_widget(self):
        ...
        self.z_limit = (
            self.instrument.sample_pose.get_travel_limits("y")
            if not self.simulated
            else {"y": [-10000, 10000]}
        )
        self.z_limit["y"] = [round(x * 1000) for x in self.z_limit["y"]]
        self.z_range = self.z_limit["y"][1] + abs(
            self.z_limit["y"][0]
        )  # Shift range up by lower limit so no negative numbers
        self.move_stage["up"] = QLabel(
            f'Upper Limit: {round(self.z_limit["y"][0])}'
        )  # Upper limit will be the more negative limit
