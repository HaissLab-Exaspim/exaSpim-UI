...
class VolumetericAcquisition(WidgetBase):
    ...
    def volumeteric_imaging_button(self):
        ...
        self.volumetric_image = {
            "start": QPushButton("Start Volumetric Imaging"),
    ...
    def run_volumeteric_imaging(self):
        ...
        self.volumetric_image["start"].blockSignals(
            True
        )  # Block release signal so transferring json doesn't start

        if self.instrument.livestream_enabled.is_set():
            self.error_msg(
                "Livestream", "Livestream is still set. Please stop livestream"
            )
        ...
        self.run_worker.finished.connect(
            lambda: self.end_scan()
        )  # Napari threads have finished signals
    ...
    def end_scan(self):
        ...
        dest = (
            str(self.instrument.img_storage_dir)
            if self.instrument.img_storage_dir is not None
            else str(self.instrument.cache_storage_dir)
        )
    ...
    def progress_bar_widget(self):
        ...
        self.progress["bar"].setStyleSheet(
    ...
    def _progress_bar_worker(self):
        ...
        QtCore.QMetaObject.invokeMethod(
            self.progress["bar"], "setHidden", QtCore.Q_ARG(bool, False)
        )
        QtCore.QMetaObject.invokeMethod(
            self.progress["end_time"], "setHidden", QtCore.Q_ARG(bool, False)
        )
        QtCore.QMetaObject.invokeMethod(
            self.progress["bar"], "setValue", QtCore.Q_ARG(int, 0)
        )
        x, y, z = self.instrument.get_tile_counts(
        ...
        for ch in self.cfg.channels:
            ...
            est_run_time += self.cfg.get_channel_cycle_time(ch) * (
                z / self.cfg.get_binning(ch)
            )
        ...
        while self.instrument.acquiring_images:
            pct = (
                (self.instrument.frame_index / total_tiles)
            ...
            if self.instrument.curr_tile_index == 0:
                completion_date = self.instrument.start_time + timedelta(
                    days=est_run_time
                )

            else:
                total_time_days = (
                    self.instrument.tile_time_s * time_scale
                )  # +tile_transfer_time_days
                completion_date = self.instrument.start_time + timedelta(
    ...
    def overwrite_warning(self):
        ...
        msgBox.setIcon(QMessageBox.Information)
        msgBox.setText(
            "Running Acquisition will overwrite files. Are you sure you want to do this?"
        )
    ...
    def waveform_graph(self):
        ...
        self.colors = np.random.randint(
            0, 255, [12, 3]
        )  # rework this so the colors are set and laser colors are consistent
    ...
    def waveform_update(self):
        ...
        t = np.linspace(
            0,
            self.cfg.get_channel_cycle_time(self.cfg.channels[0]),
            len(voltages_t[0]),
            endpoint=False,
        )  # Calculate time
        ...
        for index, ao_name in enumerate(self.cfg.n2c.keys()):
            self.waveform["graph"].addLegend(
                offset=(365, 0.5), horSpacing=20, verSpacing=0, labelTextSize="8pt"
            )
            self.waveform["graph"].plot(
                t,
                voltages_t[index],
                name=ao_name,
    ...
    def limit_widget(self):
        ...
        for direction in directions:
            ...
            self.min_max_widgets[direction + "min"] = QPushButton(
                f"Set {direction} min limit"
            )
            self.min_max_widgets[direction + "min"].clicked.connect(
                lambda pushed=None, direction=direction, extreme="min": self.set_limit(
                    pushed, direction, extreme
                )
            )
            self.min_max_widgets[direction + "minlabel"] = QLineEdit(":")
            self.min_max_widgets[direction + "minlabel"].editingFinished.connect(
                (
                    lambda entered=True, direction=direction, extreme="min": self.edit_limit(
                        entered, direction, extreme
                    )
                )
            )

            self.min_max_widgets[direction + "minlabel"].setInputMask(":xxxxxxxx um")
            self.min_max_widgets[direction + "max"] = QPushButton(
                f"Set {direction} max limit"
            )
            self.min_max_widgets[direction + "maxlabel"] = QLineEdit(":")
            self.min_max_widgets[direction + "maxlabel"].editingFinished.connect(
                (
                    lambda entered=True, direction=direction, extreme="max": self.edit_limit(
                        entered, direction, extreme
                    )
                )
            )
            ...
            self.min_max_widgets[direction + "max"].clicked.connect(
                lambda pushed=None, direction=direction, extreme="max": self.set_limit(
                    pushed, direction, extreme
                )
            )
        ...
        min_widgets = self.create_layout(
            struct="H", **{k: v for k, v in self.min_max_widgets.items() if "min" in k}
        )
        max_widgets = self.create_layout(
            struct="H", **{k: v for k, v in self.min_max_widgets.items() if "max" in k}
        )
        calculate_widget = self.create_layout(
            struct="H",
            button=self.min_max_widgets["calculate"],
            label=self.min_max_widgets["calculate label"],
        )
        calculate_widget.setMinimumHeight(50)
        return self.create_layout(
            struct="V",
            min_label=self.min_max_widgets["Min_limit"],
            min_widgets=min_widgets,
            max_label=self.min_max_widgets["Max_limit"],
    ...
    def set_limit(self, pushed, direction, extreme):
        ...
        self.min_max_widgets[direction + extreme + "label"].setText(
            f": {position[direction]/10}"
        )
        if extreme == "min":
    ...
    def calculate_scan_position(self):
        ...
        x_grid_step_um, y_grid_step_um = self.instrument.get_xy_grid_step(
            self.cfg.tile_overlap_x_percent, self.cfg.tile_overlap_y_percent
        )

        start_position = {
            "x": round(centroid["x"] - ((x / 2) * (x_grid_step_um)), 1),
        ...
        self.min_max_widgets["calculate label"].setText(
    ...
    def exceed_stage_limit_check(self, start_pos_um: dict = None, volume: dict = None):
        ...
        x, y, z = self.instrument.get_tile_counts(
            ...
            self.cfg.tile_overlap_y_percent,
            self.cfg.z_step_size_um,
            self.cfg.volume_x_um if volume is None else volume["x"],
            self.cfg.volume_y_um if volume is None else volume["y"],
            self.cfg.volume_z_um if volume is None else volume["z"],
        )
        x_grid_step_um, y_grid_step_um = self.instrument.get_xy_grid_step(
            self.cfg.tile_overlap_x_percent, self.cfg.tile_overlap_y_percent
        )
        ...
        for k in limits_um.keys():
            ...
            if (
                not limits_um[k][0] < end_pos < limits_um[k][1]
                or not limits_um[k][0] < start_pos_um[k] < limits_um[k][1]
            ):
                limit_exceeded.append(k)
        if limit_exceeded != []:
            self.error_msg(
                "CAUTION",
                "Starting stage at this position with "
