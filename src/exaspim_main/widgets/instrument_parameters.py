...
class InstrumentParameters(WidgetBase):
    ...
    def scan_config(self, config: object, x_game_mode: bool = False):
        ...
        imaging_specs_widgets = (
            {}
        )  # dictionary that holds layout of attribute labels/input pairs
        ...
        for attr in directory:
            ...
            if isinstance(value, list):
                ...
                if prop_obj.fset is not None and prop_obj.fget is not None:
                    ...
                    if (
                        attr != "image_dtype"
                        and attr != "tile_size_x_um"
                        and attr != "tile_size_y_um"
                    ):

                        self.imaging_specs[attr].editingFinished.connect(
                            lambda obj=config, var=attr, widget=self.imaging_specs[
                                attr
                    ...
                    imaging_specs_widgets[attr] = self.create_layout(
                        struct="H",
    ...
    def joystick_remap_tab(self):
        ...
        for axis in self.joystick_axes.keys():
            ...
            if JoystickInput[axis] in joystick_mapping.values():
                current_text = list(joystick_mapping.keys())[
                    list(joystick_mapping.values()).index(JoystickInput[axis])
                ]
            ...
            label = QLabel(f"{axis}:")
            combobox_layout[axis] = self.create_layout(
                struct="H", label=label, combobox=self.axis_combobox[axis]
            )
            self.axis_combobox[axis].currentTextChanged.connect(
                lambda index=None, axis=axis: self.change_joystick_mapping(index, axis)
            )
        ...
        right_left = self.create_layout(
            struct="H", left=left, right=right, box=combobox_layout["JOYSTICK_X"]
        )
        ...
        up_down = self.create_layout(
            struct="H", up=up, down=down, box=combobox_layout["JOYSTICK_Y"]
        )

        left_dial = self.create_layout(
            struct="H", dial=QDial(), box=combobox_layout["Z_WHEEL"]
        )
        right_dial = self.create_layout(
            struct="H", dial=QDial(), box=combobox_layout["F_WHEEL"]
        )

        all = self.create_layout(
            struct="V", ud=up_down, rl=right_left, ldial=left_dial, rdial=right_dial
        )
    ...
    def change_joystick_mapping(self, index, joystick_axis):
        ...
        if stage_ax == "NONE":
            ...
            self.instrument.tigerbox.bind_axis_to_joystick_input(
                **{self.joystick_axes[joystick_axis]: JoystickInput.NONE}
            )
    ...
    def brain_orientation_widget(self):
        ...
        for pos in orientation:
            img = cv2.imread(str(BIN_PATH / "mid-sagittal-brain.png"))
            label = {
                "x": "Posterior_to_anterior",
            ...
            img_widget = QLabel()
            img_widget.setPixmap(
                QPixmap(pixmap.scaled(150, 150, QtCore.Qt.KeepAspectRatio))
            )
            set_button = QRadioButton()
            set_button.toggled.connect(
                lambda state=2, orientations=label, key=pos: self.set_brain_orientation(
                    state, orientations, key
                )
            )
            self.orientaion_widget[pos] = self.create_layout(
                "V", image=img_widget, box=set_button
            )

        img_widget = QLabel("Not Applicable")
        set_button = QRadioButton()
        set_button.toggled.connect(
            lambda state=2, orientations={
                k: "Other" for k in label.keys()
            }, key="NA": self.set_brain_orientation(state, orientations, key)
        )
        NA_widget = self.create_layout("V", label=img_widget, button=set_button)
        orientation_widget = self.create_layout(
            "V",
            top=self.create_layout("VH", **self.orientaion_widget),
            bottom=NA_widget,
        )
        self.orientaion_widget["NA"] = (
            NA_widget  # Retroactivily add NA widget to orientation widget to have correct formatting
        )
        ...
        if state:
            for k in self.orientaion_widget.keys():
                if k != key:
                    ...
                    self.orientaion_widget[k].children()[2].setChecked(
                        False
                    )  # Radiobox in layout
