...
class WidgetBase:
    ...
    def start_stop_ni(self):
        ...
        self.instrument.ni.stop(
            sleep_time=self.cfg.get_channel_cycle_time(488)
        )  # Pause to get at least one frame
        ...
        try:
            ...
            if image is None:
                self.viewer.add_image(
                    image,
                    name=f"Video {layer_num}",
                    multiscale=True,
                    scale=[
                        self.cfg.tile_specs["x_field_of_view_um"]
    ...
    def scan(
        ...
        attr: str,
        prev_key: str = None,
        QDictionary: dict = None,
        WindowDictionary: dict = None,
        wl: str = None,
        input_type: str = QLineEdit,
        subdict: bool = False,
    ):
        ...
        for keys in dictionary.keys():
            if type(dictionary[keys]) != dict:
                ...
                new_key = (
                    f"{prev_key}_{keys}" if keys in QDictionary or subdict else keys
                )
                QDictionary[new_key + "_label"], QDictionary[new_key] = (
                    self.create_widget(dictionary[keys], input_type, label=new_key)
                )
                QDictionary[new_key].editingFinished.connect(
                    (
                        lambda widget=QDictionary[new_key], kw=keys: self.config_change(
                            widget, attr, kw, wl
                        )
                    )
                )

                WindowDictionary[new_key] = self.create_layout(
                    struct="H",
                    label=QDictionary[new_key + "_label"],
    ...
    def pathFind(
        self, dictionary: dict, kw: str, path: list = None, preset_path: bool = False
    ):
    ...
    def create_widget(self, value, Qtype, label):
        ...
        if isinstance(widget_input, QDoubleSpinBox):
            ...
            widget_input.setSingleStep(0.01)
