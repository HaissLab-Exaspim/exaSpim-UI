...
class Lasers(WidgetBase):
    ...
    def laser_wl_select(self):
        ...
        remaining_wavelengths = [
            str(wavelength)
            for wavelength in self.possible_wavelengths
            if wavelength not in self.imaging_wavelengths
        ]
    ...
    def selected_wv_label(self):
        ...
        for wavelengths in self.possible_wavelengths:
            ...
            self.selected[wavelengths].setStyleSheet(
                f'QPushButton {{ background-color:{color["color"]}; color '
                f":black; }}"
            )
            self.selected[wavelengths].clicked.connect(
                lambda clicked=None, widget=self.selected[
                    wavelengths
    ...
    def scan_wavelength_params(self, wv: str):
        ...
        for k, v in dial_values.items():
            ...
            self.dials[wv][k].valueChanged.connect(
                lambda value=str(
                    self.dials[wv][k].value() / 3000
                ), widget=self.dials[  # Divide to get dec
                    wv
                ][
                    k + "value"
                ]: self.update_dial_label(
                    value, widget
                )
            )
            self.dials[wv][k + "value"].textChanged.connect(
                lambda value=self.dials[wv][k].value() / 3000, path=k.split(
                    "."
                ), dict=self.cfg.channel_specs: self.config_change(value, path, dict)
            )
            self.dial_widgets[wv][k] = self.create_layout(
                struct="V",
                label=self.dials[wv][k + "label"],
    ...
    def laser_power_slider(self):
        ...
        for wl in self.possible_wavelengths:
            ...
            self.laser_power[f"{wl} label"], self.laser_power[wl] = self.create_widget(
                value=None, Qtype=QSlider, label=f"{wl}: {value} {unit}"
            )
            ...
            self.laser_power[wl].sliderReleased.connect(
                lambda value=value, unit=unit, wl=wl, released=True: self.laser_power_label(
                    value, unit, wl, released
                )
            )
            self.laser_power[wl].sliderMoved.connect(
                lambda value=value, unit=unit, wl=wl: self.laser_power_label(
                    value, unit, wl
                )
            )
            ...
            laser_power_layout[str(wl)] = self.create_layout(
                struct="H",
